...
module.exports.createPlayer = async (serviceData) => {
  try {
    let player = new PlayerLeaderboard({ ...serviceData });
...
module.exports.getAllPlayers = async (req) => {
  try {
    ...
    let player = await PlayerLeaderboard.paginate({},{page,sort: { score: -1 }})
    return (player);
...
module.exports.getPlayerById = async ({ id }) => {
  try {
    checkObjectId(id);
    let player = await PlayerLeaderboard.findById(id);
    if (!player) {
...
module.exports.updatePlayer = async ({ id, updateInfo }) => {
  try {
    checkObjectId(id);
    let player = await PlayerLeaderboard.findOneAndUpdate(
      { _id: id },
      updateInfo,
      { new: true }
    )
